/* angularjs-portal-colorpicker v1.0.5 */
...
var AngularColorPickerController = function () {
    function AngularColorPickerController(_$scope, _$element, _$document, _$window, _$timeout, _ColorPickerOptions) {
    ...
    createClass(AngularColorPickerController, [{
        ...
        value: function initWatchers() {
            ...
            this.$scope.$watchGroup(['AngularColorPickerController.pickerDisabled', 'AngularColorPickerController.options.swatchBootstrap', 'AngularColorPickerController.options.swatchOnly', 'AngularColorPickerController.options.swatch', 'AngularColorPickerController.options.pos', 'AngularColorPickerController.options.inline', 'AngularColorPickerController.options.placeholder'], function (newValue) {
                if (newValue !== undefined) {
                    _this.initConfig();
                }
            });
        ...
        key: 'onInternalNgModelChange',
        value: function onInternalNgModelChange(event) {
        ...
        value: function initMouseEvents() {
            ...
            this.find('.color-picker-saturation').on('click', function (event) {
                _this2.onClick('saturation', event);
            });
            ...
            this.find('.color-picker-lightness').on('click', function (event) {
                _this2.onClick('lightness', event);
            });
            ...
            this.$scope.$on('$destroy', function () {
                // remove mouse events
                _this2.$document.off('mousedown', eventHandlers.mouseDown);
                _this2.$document.off('mouseup', eventHandlers.mouseUp);
                _this2.$document.off('mousemove', eventHandlers.mouseMove);

                // remove touch events
                _this2.$document.off('touchstart', eventHandlers.mouseDown);
                _this2.$document.off('touchend', eventHandlers.mouseUp);
                _this2.$document.off('touchmove', eventHandlers.mouseMove);

                // remove key events
                _this2.$document.off('keyup', eventHandlers.keyUp);

                _this2.eventApiDispatch('onDestroy');
            });
        }
    }, {
        key: 'onMouseDown',
        ...
        value: function onChange(event) {
            ...
            if (this.internalNgModel !== this.onChangeValue) {
                ...
                this.eventApiDispatch('onChange', { event: event });
            }
        }
    }, {
        key: 'onBlur',
        value: function onBlur(event) {
            ...
            this.eventApiDispatch('onBlur', { event: event });
        ...
        value: function setupApi() {
            ...
            this.api.open = function (event) {
                ...
                _this3.$timeout(function () {
                    var $picker = _this3.$element.find('.color-picker-panel');
                    var position = _this3.getAdjustedPickerPosition($picker);
                    $picker.css(position);
                });
                // force the sliders to re-caculate their position
                for (var i = 0; i < _this3.basicEventTypes.length; i++) {
                    _this3.valueUpdate(_this3.basicEventTypes[i]);
                }

                _this3.eventApiDispatch('onOpen', { event: event });
            };
            ...
            this.api.close = function (event) {
                ...
                if (!_this3.options.inline && (_this3.is_open || _this3.$element[0].querySelector('.color-picker-panel').offsetParent !== null)) {
                    ...
                    _this3.update();
                    _this3.eventApiDispatch('onClose', { event: event });
                }
            };

            this.api.clear = function (event) {
                _this3.setNgModel(null);

                _this3.eventApiDispatch('onClear', { event: event });
            };
        ...
        key: 'getAdjustedPickerPosition',
        value: function getAdjustedPickerPosition($picker) {
            var position = {};
            var wWidth = this.$window.innerWidth,
                wHeight = this.$window.innerHeight,
                eWidth = $picker.outerWidth(),
                eHeight = $picker.outerHeight(),
                _$picker$offset = $picker.offset(),
                left = _$picker$offset.left,
                top = _$picker$offset.top;


            var newLeft = Math.min(0, wWidth - (left + eWidth));
            if (newLeft !== 0) {
                position.left = newLeft;
            }

            var newTop = Math.min(0, wHeight - (top + eHeight));
            if (newTop !== 0) {
                position.top = newTop;
            }
            return position;
        ...
        value: function eventApiDispatch(name) {
            var args = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

            var listenerName = name + 'Listener';
